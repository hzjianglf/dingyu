...
dojo.provide("com.ibm.sr.widgets.AddLoadOrCreateRelationshipTarget");
...
dojo.requireLocalization("com.ibm.sr.widgets.AddLoadOrCreateRelationshipTarget", "AddLoadOrCreateRelationshipTargetResources");

dojo.declare("com.ibm.sr.widgets.AddLoadOrCreateRelationshipTarget", [dijit._Widget, dijit._Templated],{

	// The path to the widgets template
	templatePath: dojo.moduleUrl("com.ibm.sr.widgets.AddLoadOrCreateRelationshipTarget", "AddLoadOrCreateRelationshipTarget.html"),
    ...
    constructor: function (args) {
        ...
        if (args) {
            ...
            // Initialize the value of allowCreate
            if (args.allowCreate) {
                this.allowCreate = args.allowCreate;
            } else {
		...
		this.messages = dojo.i18n.getLocalization("com.ibm.sr.widgets.AddLoadOrCreateRelationshipTarget", "AddLoadOrCreateRelationshipTargetResources");
    ...
    postCreate: function() {
         ...
         * Also create an array of creatable options.
         */
        ...
        for (var i = 0; i < this.searchTypes.length; i++) {
            ...
            //If the id does not match any of the predefined entity IDs then this must be 
            //a business model URI and we will add it to the creatable list
            if (this.allowCreate && this.isCreatableObject(this.searchTypes[i].id)) {
            	entityTypeOptions.push(option);
            }
        } // FOR
        ...
        // Set the entity types on the entity types list
        this.setEntityTypes(entityTypeOptions);
    ...
    setDocumentTypes: function(/*Option[]*/ types){
        ...
        if (types && types.length > 0) {
            ...
            for (var i = 0; i < types.length; i++) {
                /*
                 * Create a new Option object into the options array for
    ...
    /*
     * The setEntityTypes function sets the list of entity types that are
     * available in the "Entity Type" combo box.
     */
    setEntityTypes: function(/*Option[]*/ types){

        // Log entry to the method
        //console.debug(">>> setEntityTypes(", types, ")");
        ...
        if (types && types.length > 0) {
            ...
            for (var i = 0; i < types.length; i++) {
                 ...
                 * because there are cross-browser compatibility issues.
                 */
                this.entityTypeList.options[i] = new Option(types[i].text, types[i].value);
            } // FOR
        } else { // IF - types && types.length > 0
            // No document types were specified.  Hide the entire div.
            this.createBOFieldSet.style.display = "none";
        }
    ...
    /*
     * Gets the value of the currently selected document type 
     *
     */
    ...
    /*
     * Gets the value of the currently selected entity type 
     *
     */
    getEntityType: function() {
    	return this.entityTypeList.value;
    },
    ...
    onCreateEntityButtonClicked: function(event) {

        // Log entry to the method
        //console.debug(">>> onCreateEntityButtonClicked(", event, ")");
        

        // Log exit from the method        
        //console.debug("<<< onCreateEntityButtonClicked");
    },
	...
	isCreatableObject : function(/* string*/ id) {
	
		// Log entry to the method
    	//console.debug(">>> isCreatableObject: " + id);
    	...
    	switch (id) {
			...
			case "GovernableObject" :
			case "ProxyObject" :
			case "QueryObject" :
			case "PropertyQuery" :
			case "PropertyQueryResult" :
			case "GraphQuery" :
			case "ObjectQuery": 
			...
			case "Module" :
			case "ExportBinding" :
			case "JMSExportBinding" :
			case "JMSImportBinding" :
			case "SCAExportBinding" :
			case "SCAImportBinding" :  
			case "ImportBinding" :
			case "SLSBImportBinding" :
			case "Export" :
			case "Import" :
			case "Interface" :
			case "SCAWSDLPortType" :
			case "WebServiceExportBinding" :
			case "WebServiceImportBinding" :
			case "OntologyClass" :
    	...
    	// Log exit to the method
    	//console.debug("<<< isCreatableObject: " + result);
