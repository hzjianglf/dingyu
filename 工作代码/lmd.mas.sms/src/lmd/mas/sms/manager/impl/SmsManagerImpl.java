package lmd.mas.sms.manager.impl;

import lmd.mas.sms.manager.SmsManager;
import lmd.mas.sms.util.SmsSendUtil;
...
//		      returnStr = changeStr(returnStr);
		     ...
		     if(bnfs){//是true就不能发送
		    	 ...
		    	 int result=SmsSendUtil.sendSm(mobileStr, content);
			      if(0==result){
			    	  returnStr="1";//发送成功
			      }else{
			    	  returnStr=result+"";
	...
	public String sendByMobile(String mobiles, String content) {
		// TODO 自动生成的方法存根
		String returnStr="";
		int result=SmsSendUtil.sendSm(mobiles, content);
	      if(0==result){
	    	  returnStr="1";//发送成功
	      }else{
	    	  returnStr=result+"";
