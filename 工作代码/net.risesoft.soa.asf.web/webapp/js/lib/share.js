...
Share.AjaxRequest = function(settings) {
	// 参数对象
	var params = settings.params === undefined ? {} : settings.params, showWaiting = settings.showWaiting === undefined
			? true
			: settings.showWaiting, showMsg = settings.showMsg === undefined ? true : settings.showMsg, timeout = settings.timeout === undefined
			? 60 * 1000
			: settings.timeout,
	// 发送请求
	waiting = null;
...
Share.ExceptionWindow = function(message, exceptionMessage) {
	var _message = message === undefined ? '发生异常!' : message == '' ? '发生异常!' : message, _exceptionMessage = exceptionMessage === undefined
			? '发生异常!'
...
Share.attachParams = function(url, params) {
	params = params || {};
 ...
 *            样式
 */
Share.openTab = function(node, url, parentTab, iconCls, params) {
	var id = node.id, title = node.text,
	// 设置在哪个元素打开
	// tabPanel = parentTab === undefined ? App.Index.tabPanel : parentTab,
	tabPanel = parentTab, css = iconCls === undefined ? node.attributes.iconCls : iconCls;
	// 参数验证
   ...
   id += '_div_panel';
	...
	if (!tab) {
		var newTab = {
			...
			autoScroll : true,
			// frame : true,
			html : '<iframe id="' + id + '-iframe" src="' + url
      ...
      if (iframe && iframe.contentWindow) {
         ...
         var oldOnLoad = innerWindow.onload;
         innerWindow.onload = function() {
				if (oldOnLoad) {
					oldOnLoad();
				}
				if (innerWindow.updateContent && typeof(innerWindow.updateContent) == 'function') {
					innerWindow.updateContent(params);
				}
			}
      }
