...
function personnelTreeNodeClick(node){
	...
	var nodeId = node.id;	
  	...
  	load({url:'file_showFile2.action?showFile=preListView2&folderUid=' + nodeId,scripts:true});			
}
/**
 * 刷新个人文件树
 */
function refreshPersonnelTree(){
	...
	uid = rtnData.uid; 
	Ext.getCmp('index-center-panel').
	load({url:'file_showFile2.action?showFile=preListView2&folderUid=' + uid,scripts:true});	
	Ext.getCmp('index-center-panel').reload();
}
...
Ext.onReady(function(){
	...
	Ext.getCmp('index-center-panel').load({url:'file_showFile2.action?showFile=preListView2&folderUid=' +
		...
		folderTreePanel.add(new Ext.Panel({
			...
			listeners: {
				activate:function(p,state){		
					...
					Ext.getCmp('index-center-panel').load({url:'file_showFile2.action?showFile=preListView2&folderUid=' + p.id,scripts:true});					
				}
			}
		}));
	}*/
...
function _left_refreshTreeAndList(folderUid){
	...
	file_imgViewPageSize();
 	Ext.getCmp('index-center-panel').
	load({url:'file_showFile2.action?showFile=preListView2&folderUid=' + folderUid,scripts:true});	
}
