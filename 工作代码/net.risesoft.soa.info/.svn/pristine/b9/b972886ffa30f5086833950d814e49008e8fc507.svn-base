...
public class InfoList extends BaseAction
{
    ...
    throws Exception
  {
    ...
    if ("delete".equals(this.operation)) {
      ...
      if (informationFiles.size() > 0)
    	    try
      {
          InformationFile informationFile;
          for(Iterator iterator = informationFiles.iterator(); iterator.hasNext(); ifm.delete(informationFile.getId()))
          {
              informationFile = (InformationFile)iterator.next();
              String path = (new StringBuilder(String.valueOf(ConfigUtil.filePath))).append(informationFile.getRealPath()).append(informationFile.getId()).append(informationFile.getFileNameExt()).toString();
              ...
              {
                  path = (new StringBuilder(String.valueOf(ConfigUtil.filePath))).append(informationFile.getRealPath()).append(informationFile.getId()).append(".swf").toString();
                  file = new File(path);
                  if(file.exists())
                      file.delete();
              }
      ...
      {
          response.getWriter().write("{\"success\":false,\"message\":\"\u5220\u9664\u4FE1\u606F\u5931\u8D25!\"}");
          return "none";
      }
      try
      {
        ...
        response.getWriter().write("{\"success\":true,\"message\":\"删除信息成功!\"}");
      } catch (Exception e) {
        e.printStackTrace();
        response.getWriter().write("{\"success\":false,\"message\":\"删除信息失败!\"}");
        return "none";
    ...
    if ("disable".equals(this.operation)) {
        HttpServletResponse response = getServletResponse();
        response.setContentType("text/html;charset=UTF-8");
        ...
        try
        {
          ...
          if (informationIndex != null) {
            ...
            Query query = em.createNativeQuery("update  " + tableName + " set acshow='1' where id = '" + this.id + "'");
          ...
          response.getWriter().write("{\"success\":true,\"message\":\"禁用资源成功!\"}");
        } catch (Exception e) {
          e.printStackTrace();
          response.getWriter().write("{\"success\":false,\"message\":\"禁用资源失败!\"}");
          return "none";
        }
        return "none";
      }
      ...
      String tempSql = "";
       ...
       if("all".equals(shortacnum)){
    	      tempSql = tempSql + " from info_main_index," + tableName + 
    	    	        ...
    	    	        ".id and info_main_index.infoID = '" + this.infoID + "' and  acshow='0' ";
       }
       else{
    	  // int shlength=shortacnum.length();
    	      tempSql = tempSql + " from info_main_index," + tableName + 
    	    	        " where info_main_index.id = " + tableName + 
    	    	        ".id and info_main_index.infoID = '" + this.infoID + "' and  acshow='0' and substr( shortacnum ,0,"+shortacnum.length()+") ='"+shortacnum+"'";  
       }
