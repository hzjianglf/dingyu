...
<table border=0 width=90% align=center>
    ...
    <tr>
	 <%if (actionName.contains("create")&&shortacnum.length()==9){%>
	 <td align=right height=40>
	    	<a href="javascript:infoAdd('<%=infoID%>');" class="link2">新增</a>
	    </td>
		<%} if(actionName.contains("acshow")){%>
	    <td align=right height=40>
	    	<a href="javascript:ACShow();" class="link2">历史</a>
	    </td>
		<%}else{
			%>
			<td height=10>&nbsp;</td>
...
<script type="text/javascript">
	$(document).ready(function() {  
		...
		$("#talkList").load("/info/infoList.action",{"infoID": "<%=infoID%>","shortacnum":"<%=shortacnum%>", "action":"<%=actionName%>", "search": "<%=search%>","acid":"<%=acid%>"});
	}); 
	...
	function pageselectCallback(start, jq){	
		//$("#talkList").empty();
		$("#talkList").load("/info/infoList.action?" + $('#form1').formSerialize(),{"infoID": "<%=infoID%>", "shortacnum":"<%=shortacnum%>","action":"<%=actionName%>", "pageNo": start, "search": "<%=search%>","acid":"<%=acid%>"});
	}

	function searchInformation(){
		$("#talkList").load("/info/infoList.action?" + $('#form1').formSerialize(),{"infoID": "<%=infoID%>", "shortacnum":"<%=shortacnum%>","action":"<%=actionName%>", "search": "<%=search%>","acid":"<%=acid%>"});
	}
</script>
...
function ACShow(){
	$("#infoMainDiv").empty();
  $("#infoMainDiv").load("/info/display/panel2.jsp", {"infoID":"<%=infoID%>","shortacnum":"<%=shortacnum%>", "search":"<%=search%>","acid":"<%=acid%>"});
}
function infoAdd(id){
	$("#infoMainDiv").empty();
    $("#infoMainDiv").load("/info/display/infoAdd.jsp", {"infoID": id,"shortacnum":"<%=shortacnum%>", "operation":"create","acid":"<%=acid%>"});
}
function infoDisable(id){
	var message = '是否要禁用此资源?';
	var bool = window.confirm(message);
    if (bool){
		$.post('/info/infoList.action', 
				{ operation: 'disable', id: id},
				function(data){
					if (data.success){
						...
						$.post('/csbwork/mnicontro/mfstatusoper.do?infoid='+id+'&oper=disable');// add by sunming 20150921												
					}else{
...
function infoAdd2(infoid,id){
	$("#infoMainDiv").empty();
    $("#infoMainDiv").load("/info/display/infoAdd.jsp", {"infoID": infoid,"shortacnum":"<%=shortacnum%>", "operation":"create","myid":id,"acversion":"acversion","acid":"<%=acid%>"});
}
function infoDelete(id){
	var message = '是否要删除此信息?';
	var bool = window.confirm(message);
    if (bool){
		$.post('/info/infoList.action', 
...
function infoEdit(id){
	$("#infoMainDiv").empty();
	$("#infoMainDiv").load("/info/display/infoAdd.jsp", {"id":id, "operation":"edit","acid":"<%=acid%>"});
}
