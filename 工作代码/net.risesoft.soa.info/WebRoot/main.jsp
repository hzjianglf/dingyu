...
<UL id="topNav" class="topNav right">
  ...
  %>
  	<%if (i == 0){ %>
  		<LI class="topNavItem active" style="_cursor: pointer; cursor: pointer;" onclick="javascript:showInfoList(this);" uuid="<%=resource.getUID()%>" name="<%=resource.getName()%>">
  	<%}else{%>
  		<LI class="topNavItem" style="_cursor: pointer; cursor: pointer;" onclick="javascript:showInfoList(this);" uuid="<%=resource.getUID()%>">
  	<%}%>
  	<A><U><%=resource.getName()%></U></A>
  	</LI>
  <%}%>
</UL>
</DIV>
...
<table width="100%" border="0" align="center" cellspacing="0" style="vertical-align:top">
  <tr>
  	<td height="100%">
  		<table border=0 cellpadding="0" cellspacing="0" align="center" width="100%" style="background-color: #e0e9e9;border: 10px solid #e0e9e9; border-radius: 0px 0px 5px 5px; -moz-border-radius:0px 0px 5px 5px; -webkit-border-radius:0px 0px 5px 5px;">
  		<tr>
  						...
  						<td width=178 height=42 style="background:url(images/head.png) no-repeat center center;" align="center">
  							<table border=0>
  								<tr>
  									...
  									<td style="color:#FFFFFF; font-weight:bold; font-size:14pt; *padding-top:3px; padding-top:3px \0;">资源发布</td>
  								</tr>
  							</table>
  						</td>
  					</tr>
...
$(document).ready(function() {
	...
	if ($(".active").attr("uuid") != null){
		$("#leftList_div").load("list.jsp",{"id":$(".active").attr("uuid")})
	}
...
function showInfoList(obj){
	...
	$(obj).addClass("active"); 
	$("#leftList_div").load("list.jsp",{"id":$(obj).attr("uuid")})
}
