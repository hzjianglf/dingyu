...
<div class="form">
	...
	<div class="div-ranks">
		<label for="ranks">设置密码</label><input type="password" id="password" class="infos" placeholder="设置密码"  /><span id="checkPwdForm" style="color:red;">6-16位字母或数字！</span>
	</div>
	...
	send:function(){
			...
			var val = $('#phone').val().replace(/\s+/g,""); //获取输入手机号码
				...
				if(!numbers.test(val) || val.length ==0){
					...
					$.ajax({
						url:"/mall/pm/user/getVerifyCode.do",
						data:{							
							"loginName":loginName,
							"sign":"1",
							},
...
function conform(){
	...
	if(!reg.test(document.getElementById("password").value) || document.getElementById("password").value.length > 16 || document.getElementById("password").value.length < 6){
		$("#checkPwdForm").show();		
		return;
	}
	...
	$.ajax({
		...
		if(true == data.header.success){
			...
			window.location="/mall/pm/user/manage.do?tokenId=" + tokenId;
		}else{
			/* if($('.div-phone').find('span').length == 0){
				$('.div-phone').append('<span class="error">用户名或密码错误</span>');
			}else{
