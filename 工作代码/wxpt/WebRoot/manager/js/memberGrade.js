$(function() {
	...
	$('#tt').datagrid({
		...
		collapsible : true,
		singleSelect : true,
		url : '../site/member-grade!getGrade',
		loadMsg : '数据装载中......',
		sortName : 'code',
		sortOrder : 'desc',
		remoteSort : false,
...
function add() {
	$.ajax({
		type : "POST",
		url : "../site/member-grade!addGrade",
...
function UpdateGrade() {
	...
	if (select) {
		...
		$("#addUser").css("visibility", "visible");
		$("#addUser").show();
		$("#gradeName").val(select.memberGradeName);
		$("#gradeJf").val(select.gradeJifen);
		$("#gradeId").val(select.memberGradeId);
...
function update() {
	$.ajax({
		type : "POST",
		url : "../site/member-grade!updateGrade",
		data : '' + $("#addUser").serialize(),
		dataType : "json",
		success : function() {
