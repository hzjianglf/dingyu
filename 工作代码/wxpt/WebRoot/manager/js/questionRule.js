$(function() {
	...
	$('#tt').datagrid({
		...
		collapsible : true,
		singleSelect : true,
		url :' ../site/manage-question-rule!selectRule',
		loadMsg : '数据装载中......',
		sortName : 'code',
		sortOrder : 'desc',
		remoteSort : false,
...
function AddGrade() {
	...
	$.ajax({
		type : "POST",
		url : "../site/manage-question-rule!panduan",
...
function UpdateGrade() {
	...
	if (select) {
		...
		$("#addUser").css("visibility", "visible");
		$("#addUser").show();
		$("#questionNum").val(select.questionNum);
		$("#system").val(select.questionTishi);
		$("#prize").val(select.awards);
		$("#ruleId").val(select.questionRuleId);
		$("#fangqi").val(select.fangqi);
