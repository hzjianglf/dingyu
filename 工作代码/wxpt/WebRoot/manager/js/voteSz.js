$(function() {
	...
	$('#tt').datagrid({
		...
		collapsible : true,
		singleSelect : true,
		url :' ../site/manage-vote!getVoteSz',
		loadMsg : '数据装载中......',
		sortName : 'code',
		sortOrder : 'desc',
		remoteSort : false,
...
function add() {
	$.ajax({
		type : "POST",
		url : "../site/manage-vote!addVoteSheZhi",
		data : '' + $("#addUser").serialize(),
		dataType : "json",
		success : function() {
...
function UpdateGrade() {
	...
	if (select) {
		...
		$("#addUser").css("visibility", "visible");
		$("#addUser").show();
		$("#tpc").val(select.meiJiang);
		$("#tpId").val(select.questionTishiId);
		$("#ytp").val(select.choujiang);
...
function update() {
	$.ajax({
		type : "POST",
		url : "../site/manage-vote!updateTishi",
		data : '' + $("#addUser").serialize(),
		dataType : "json",
		success : function() {
