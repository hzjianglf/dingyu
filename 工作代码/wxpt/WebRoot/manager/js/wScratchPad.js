...
//new_image.src = document.getElementById("imageE").src;
alert( $("#prvFileName").val());
(function($)
{
	$.fn.wScratchPad = function(option, settings, fileImages)
	{
		fileImage = fileImages;
	...
	{
		width			: 378,					// set width - best to match image width
		height			: 265,					// set height - best to match image height
		image			: $("#prvFileName").val(),			// set image path
		image2			: null,					// set overlay image path - if set color is not used
		color			: '#CDD1D4',			// set scratch color - if image2 is not set uses color
		overlay			: 'none',				// set the type of overlay effect 'none', 'lighter' - only used with color
		size			: 80,					// set size of scratcher
